...
from models import train_linear_regression, train_random_forest, evaluate_model,train_polynomial_regression,train_decision_tree,train_xgboost,train_svr
...
xgb_regressor = train_xgboost(X_train, y_train)
...
r2_svr, mse_svr = evaluate_model(y_test, y_pred_svr)
r2_xgb, mse_xgb = evaluate_model(y_test, y_pred_xgb)
...
print(f"SVR R²: {r2_svr:.2f}, MSE: {mse_svr:.2f}")
print(f"XGBoost R²: {r2_xgb:.2f}, MSE: {mse_xgb:.2f}")
